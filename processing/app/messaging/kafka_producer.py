...
class KafkaMessageProducer:
    ...
    def send_simulation_progress(self, simulation_id, progress_percentage, status_message):
        ...
        try:
            ...
            logger.info(
                f"Sending simulation_progress message for simulation {simulation_id}: {progress_percentage}% - {status_message}"
            )

            future = self.producer.send(self.topic, message)
            self.producer.flush()

            if progress_percentage % 20 == 0:
                record_metadata = future.get(timeout=10)
                logger.debug(
                    f"Progress message sent to {record_metadata.topic}, partition {record_metadata.partition}"
                )
...
class FewsKafkaMessageProducer(KafkaMessageProducer):
    def __init__(self, resource_provider: str):
        ...
        try:
            ...
            raise
    # The rest of the methods are inherited from KafkaMessageProducer without changes